...
import pickle
from utility_method import computeNashEquilibriumState, saveToCSVfile
...
def sortNetworkList(networkGraph):
    '''
...
def getDeviceToMove(fromNetworkID, deviceList, problemInstance, currentTimeSlot, networkDataRate, numDevicePerNetwork):
    '''
    ...
    for device in deviceList:
        ...
        if not problem_instance.isNetworkAccessible(problemInstance, currentTimeSlot, fromNetworkID, device):
            return device, 0
    # to handle case when the device was just transitting through the current network and the number of devices that was associated to the network is zero
...
def moveDevice(networkOnPath, maxNumDeviceToMove, networkGraph, problemInstance, currentTimeSlot, networkDataRate, numDevicePerNetwork, NE):
    '''
    description:
    ...
    for toNetwork in networkOnPath[1:]:     # consider every edge along the path between fromNetworkID and toNetworkID
        for deviceID in deviceMovedList:
            if problem_instance.isNetworkAccessible(problemInstance, currentTimeSlot, toNetwork, deviceID): networkGraph[fromNetwork][toNetwork]['device_list'].add(deviceID)   # see below (*)
        for i in range(numDeviceToMove):
            deviceID, currentGain = getDeviceToMove(fromNetwork, networkGraph[fromNetwork][toNetwork]['device_list'], problemInstance, currentTimeSlot, networkDataRate, numDevicePerNetwork)
    ...
    for device in moveSet:
        ...
        for network in range(1, len(networkDataRate) + 1):
            if network != fromNetwork and problem_instance.isNetworkAccessible(problemInstance, currentTimeSlot, network, device):
...
def computeDistanceToNashEquilibrium(problemInstance, numTimeSlot, networkData, numDevice):
    '''
    ...
    print("in compute distance to Nash equilibrium")
    ...
    for row in networkData: # each time slot
        ...
        if problemInstance == "continuous" or problem_instance.changeInEnvironment(problemInstance, timeSlot):
            ...
            # else:
            networkDataRate = problem_instance.getNetworkDataRate(problemInstance, timeSlot); numNetwork = len(networkDataRate)
            NElist = problem_instance.getNashEquilibriumState(problemInstance, timeSlot)
            networkIDlist = list(range(1, numNetwork + 1))
            deviceListPerNetwork = problem_instance.getDeviceListPerNetwork(problemInstance, timeSlot)

        numDevicePerNetwork = [row[2 + (2 * numNetwork) + i] for i in range(numNetwork)]                   # construct list with number of devices per network
        # print("numDevicePerNetwork:", numDevicePerNetwork)
        devicePerNetwork = [row[2 + (3 * numNetwork) + i] for i in range(numNetwork)]       # construct list of sets of devices that selected each network
        networkGraph = buildGraph(networkIDlist, devicePerNetwork, deviceListPerNetwork)    # build graph of networks and devices
        sortedNetworkIDList = sortNetworkList(networkGraph)                                 # sort list of network IDs in order of accessibility
                        ...
                        while j < len(numDeviceDiff):
                            ...
                            if numDeviceDiff[toNetworkID - 1] > 0:              # check if device(s) must be moved to network with ID toNetworkID
                                ...
                                if (len(networkOnPath) - 1) == pathLength:  # the length is equal to the current length being considered
                                    ...
                                    numDeviceToMove, tmpMoveSet = moveDevice(networkOnPath, maxNumDeviceToMove, networkGraph, problemInstance, timeSlot, networkDataRate, numDevicePerNetwork, NE)
...
def main():
    ...
    for runIndex in range(1, numRun+1):
        ...
        distancetoNE = computeDistanceToNashEquilibrium(problemInstance, numTimeSlot, networkData, numDevice)
        print(distancetoNE)
        saveToCSVfile(runDir + "distanceToNE_percentage.csv", [["timeslot", "distance"]] + [[timeIndex + 1, distancetoNE[timeIndex]] for timeIndex in range(len(distancetoNE))])
    ...
    saveToCSVfile(rootDir + "distanceToNE_percentage_median.csv", [["timeslot", "distance"]] + [[timeIndex + 1, distanceToNE_combined[timeIndex]] for timeIndex in range(len(distanceToNE_combined))])
    # end main
